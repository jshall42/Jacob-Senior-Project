...
data_no_na <- data_no_na %>% select(-cluster)
existing_data <- data_no_na
...
updated_data <- updated_data |> select(-c("PCIAT.PCIAT_01", "PCIAT.PCIAT_02", "PCIAT.PCIAT_03", "PCIAT.PCIAT_04", 
...
split <- initial_split(updated_data, prop = 0.70)
